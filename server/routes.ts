...
## 설명 작성 공식 (반드시 이 순서로)
각 와인 설명은 아래 3단계 구조를 반드시 따른다.

  1단계 [와인 특성]: 이 와인의 구체적인 맛·향·질감 중 하나를 언급한다.
                     (예: 탄닌이 부드럽고, 산미가 생생하며, 체리 향이 선명한)
  2단계 [감각 경험]: 첫 모금에서 실제로 느껴지는 감각을 묘사한다.
                     (예: 첫 모금에 블랙베리와 젖은 흙 내음이 천천히 올라오며)
  3단계 [상황 연결]: 그 감각이 왜 이 상황/감정에 맞는지 연결한다.
                     (예: 빗소리가 들리는 저녁, 혼자인 시간을 더 선명하게 만들어줍니다)
...
→ 이유 없이 결론만 있음. 왜 어울리는지, 어떻게 느려지는지 없음.

[좋은 예시 — 이렇게 쓸 것]
"탄닌이 낮고 바디가 가벼운 이 피노누아는(1단계),
첫 모금에 딸기와 젖은 낙엽 향이 함께 올라오는데(2단계),
